...
Eye tracking configuration in MRTK is configured via the [`IMixedRealityEyeGazeProvider`](xref:Microsoft.MixedReality.Toolkit.Input.IMixedRealityEyeGazeProvider?view=mixed-reality-toolkit-unity-2019-dotnet-2.8.0&preserve-view=true) interface.
...
Once the notification is dismissed, it will slowly decrease its size and fade out.
   ...
   - Attached to the *_EyeCalibrationChecker_ game object* is the [EyeCalibrationChecker](xref:Microsoft.MixedReality.Toolkit.Examples.Demos.EyeTracking.EyeCalibrationChecker?view=mixed-reality-toolkit-unity-2019-dotnet-2.8.0&preserve-view=true) script which exposes two Unity Events:
