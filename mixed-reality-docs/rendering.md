...
* The max viewport size is a property of the [HolographicDisplay](https://docs.microsoft.com/uwp/api/Windows.Graphics.Holographic.HolographicViewConfiguration#Windows_Graphics_Holographic_HolographicViewConfiguration). HoloLens is set to the maximum viewport size, which is 720p (1268x720), by default.
...
* The lowest supported viewport size is 50% of 720p, which is 360p (634x360). On HoloLens, this is a ViewportScaleFactor of 0.5. If the value of the ViewportScaleFactor property is lower than 0.5, the viewport scale factor used to generate HolographicFrames will be clamped to 0.5.
...
* The lowest supported viewport size is 634x412. On HoloLens 2, this is a ViewportScaleFactor of approximately 0.44 when the default render target size is in use.
