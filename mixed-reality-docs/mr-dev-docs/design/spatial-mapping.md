...
The built-in NavMesh functionality in Unity cannot be used by default with spatial mapping surfaces. This is because spatial mapping surfaces aren't known until the application starts, to use Unity's navmesh for pathfinding you can install [NavMeshComponents](https://github.com/Unity-Technologies/NavMeshComponents). Using the components provided you can build a navmesh during runtime. Also note that, the spatial mapping system won't provide [information about surfaces far away](spatial-mapping.md#the-environment-scanning-experience) from the user's current location. So the application must 'remember' surfaces itself if it's to build a map of a large area. You can also increase the observation extents setting in the spatial awareness profile, which increases the area on which you can build your navmesh.
