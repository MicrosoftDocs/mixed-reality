...
Extended eye tracking APIs provide access to existing eye tracking values and new features such as individual (left and right) eye gaze vectors, supported frame rates, and the ability to set the eye tracking frame rate to 30, 60 or 90fps. These APIs will be protected behind a custom capability and developers will need approval from Microsoft to use them. To use the Extended Eye Tracking API, refer to our developer guide for using [Extended Eye Tracking APIs](../develop/native/extended-eye-tracking.md). 
