...
As with local anchors, World Locking Tools can use a group of Azure Spatial Anchors to lock entire coordinate spaces relative to the physical world, rather than using individual anchors to lock individual objects. World locking the entire space not only provides an environment more conducive to precise layout, but is also more efficient in both developer time and runtime resources.

Further information and samples leveraging Azure Spatial Anchors to share coordinate systems across HoloLens, Android, and iOS devices, as well as persisting spaces across sessions, may be found in the [World Locking Tools documentation](https://microsoft.github.io/MixedReality-WorldLockingTools-Unity/DocGen/Documentation/HowTos/WLT_ASA.html).
