...
| [Managing power and thermals](unity/managing-power-and-thermals.md) |  Download PowerThermalNotification SDK via the [Mixed Reality Feature Tool](/windows/mixed-reality/develop/unity/welcome-to-mr-feature-tool). | Unity C#, Win32 C++ | HoloLens 2 uses PowerThermal Notification SDK to subscribe to notification events.  You can implement your own custom actions to allow the HoloLens 2 devices to operate in warm environments with heavy performance requirements. See the [PowerThermalNotification API reference](/dotnet/api/microsoft.mixedreality.powerthermalnotification?view=mixedreality-powerthermalnotification-sdk-dotnet). |
