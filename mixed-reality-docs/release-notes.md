...
* **H.265 HEVC Video Playback does not work** - Applications that attempt to playback H.265 videos will receive an error message. The workaround is to use the [Device Portal](https://docs.microsoft.com/en-us/windows/uwp/debug-test-perf/device-portal-hololens) Views->Apps to remove the HEVC application and install the latest [HEVC Video Extension](https://www.microsoft.com/en-us/p/hevc-video-extensions/9nmzlz57r3t7) from the Store. We are investigating the issue. 
