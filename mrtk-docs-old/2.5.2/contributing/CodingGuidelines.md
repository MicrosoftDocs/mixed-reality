...
1. Use C#'s [`Path` APIs](https://docs.microsoft.com/dotnet/api/system.io.path?view=netframework-4.8add&preserve-view=true) whenever possible such as `Path.Combine` or `Path.GetFullPath`.
1. Use / or [`Path.DirectorySeparatorChar`](https://docs.microsoft.com/dotnet/api/system.io.path.directoryseparatorchar?view=netframework-4.8add&preserve-view=true) instead of \ or \\\\.
